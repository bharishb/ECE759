...
#include "LSTM.cuh"
template<typename T>
__global__ void mat_hadamard_kernel(const T* A, const T* B, T* C, int p, int q, int accum_mode) {

        //Addition by reading from Global Memory

printf("kernel call hadamard: %d, blockdim.x = %d, blockdim.y = %d\n", threadIdx.x, blockDim.x, blockDim.y);
...
__host__ void LSTM::mat_hadamard_gpu(const float *A, const float *B, float *C, int p, int q){
	...
	//kernel Launch
	mat_hadamard_kernel<float><<<dimGrid, dimBlock>>>(A, B, C, p, q, 0);
